"""SendGrid email client — 100/day free tier."""
import os
import requests
import base64

SENDGRID_API_KEY = os.environ.get("SENDGRID_API_KEY", "")
SENDGRID_FROM = os.environ.get("SENDGRID_FROM", "")
SENDGRID_ENDPOINT = "https://api.sendgrid.com/v3/mail/send"


def _looks_like_html(text):
    if not text:
        return False
    lowered = text.lstrip()[:200].lower()
    return lowered.startswith("<!doctype") or lowered.startswith("<html") or "<body" in text.lower()


def _html_to_plain(html):
    import re as _re
    import html as _html
    text = _re.sub(r"<(style|script)[^>]*>.*?</\1>", "", html, flags=_re.DOTALL | _re.IGNORECASE)
    text = _re.sub(r"<br\s*/?>", "\n", text, flags=_re.IGNORECASE)
    text = _re.sub(r"</(p|div|tr|h[1-6])>", "\n", text, flags=_re.IGNORECASE)
    text = _re.sub(r"<[^>]+>", "", text)
    text = _html.unescape(text)
    text = _re.sub(r"\n\s*\n\s*\n+", "\n\n", text)
    return text.strip()


def send_email(account, to_addr, subject, body_text, thread_id=None,
                in_reply_to=None, references=None,
                image_bytes=None, image_filename=None, image_mime=None,
                image_placement="attachment"):
    """Send via SendGrid API. Returns (message_id, thread_id)."""
    if not SENDGRID_API_KEY or not SENDGRID_FROM:
        raise RuntimeError("SendGrid not configured. Set SENDGRID_API_KEY and SENDGRID_FROM.")

    if _looks_like_html(body_text):
        html_body = body_text
        text_body = _html_to_plain(body_text)
    else:
        html_body = None
        text_body = body_text

    if image_bytes and image_placement == "inline":
        b64 = base64.b64encode(image_bytes).decode()
        mime = image_mime or "image/png"
        img_tag = f'<br><img src="data:{mime};base64,{b64}" style="max-width:100%">'
        if html_body:
            if "</body>" in html_body.lower():
                idx = html_body.lower().rindex("</body>")
                html_body = html_body[:idx] + img_tag + html_body[idx:]
            else:
                html_body = html_body + img_tag

    from_email = SENDGRID_FROM.split("<")[-1].strip(" <>") if "<" in SENDGRID_FROM else SENDGRID_FROM
    from_name = SENDGRID_FROM.split("<")[0].strip() if "<" in SENDGRID_FROM else "Manofox"

    content = []
    if text_body:
        content.append({"type": "text/plain", "value": text_body})
    if html_body:
        content.append({"type": "text/html", "value": html_body})

    payload = {
        "personalizations": [{"to": [{"email": to_addr}]}],
        "from": {"email": from_email, "name": from_name},
        "subject": subject,
        "content": content or [{"type": "text/plain", "value": text_body}],
    }

    attachments = []
    if image_bytes and image_placement == "attachment":
        b64 = base64.b64encode(image_bytes).decode()
        attachments.append({
            "content": b64,
            "type": image_mime or "image/png",
            "filename": image_filename or "image.png",
            "disposition": "attachment",
        })
    if attachments:
        payload["attachments"] = attachments

    headers = {
        "Authorization": f"Bearer {SENDGRID_API_KEY}",
        "Content-Type": "application/json",
    }

    try:
        resp = requests.post(SENDGRID_ENDPOINT, json=payload, headers=headers, timeout=30)
    except requests.RequestException as e:
        raise RuntimeError(f"SendGrid connection error: {e}")

    if resp.status_code >= 400:
        raise RuntimeError(f"SendGrid API {resp.status_code}: {resp.text[:300]}")

    # SendGrid returns empty on success, use response header for message ID
    msg_id = resp.headers.get("X-Message-Id", "sendgrid-" + str(hash(to_addr + subject))[:16])
    return msg_id, None
                  
